...
import { SSIFragmentClass } from './SSIFragmentClass';
...
describe('SSIFragment Component', () => {
  ...
  it('should render the SSI', () => {
    const fragment = render(<SSIFragmentClass id="the-id" url="/the-url" />);
  ...
  it('should keep the content which is already there', () => {
    ...
    const fragment = render(<SSIFragmentClass id="the-id" url="/the-url" isOnClient={true} />);
    const containerWithId = fragment.container.querySelector('#the-id');
  ...
  it('should fetch the url on the client when SSI did not happen', done => {
    ...
    const { container, rerender } = render(<SSIFragmentClass id="the-id" url="/the-url" isOnClient={true} />);
    ...
    rerender(<SSIFragmentClass id="the-id" url="/the-url" isOnClient={true} />);
